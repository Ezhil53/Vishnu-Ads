...
export function ArcTimeline(props) {
  ...
  const [circleContainerRotateDeg, setCircleContainerRotateDeg] = useState(() => (
    -1 * getInitialIndex() * angleBetweenMinorSteps * (lineCountFillBetweenSteps + 1) -
  ...
  useEffect(() => {
    setCircleContainerRotateDeg(
      -1 * currentIndex * angleBetweenMinorSteps * (lineCountFillBetweenSteps + 1)
      - angleBetweenMinorSteps * boundaryPlaceholderLinesCount
    );
  }, [currentIndex, angleBetweenMinorSteps, lineCountFillBetweenSteps, boundaryPlaceholderLinesCount]);
  ...
  function handleWheel(e) {
    ...
    // Only prevent default scroll if we're in the middle of the timeline
    if (!isAtStart && !isAtEnd) {
      e.preventDefault();
      e.stopPropagation();
    }

    // If at boundaries, allow natural scroll
    if (isAtStart || isAtEnd) {
      return;
    }
